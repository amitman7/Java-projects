...
public class AVLTree implements Iterable<Integer> {
	...
	protected Node insertNode(Node node, int value) {
		Deque<Object> stck = new LinkedList<Object>();
		int state = 0; // 0 - no balance, 1 - LL, 2 - RR, 3 - LR, 4 - RL
	    // Perform regular BST insertion
        if (node == null) {
        	Node insertedNode = new Node(value);
        	stck.push(insertedNode);
        	stck.push(state);
        	backStack.push(stck);
            return insertedNode;
        }
        ...
        if (balance > 1) {
        	stck = backStack.pop();
        	stck.pop(); // poping the old state
        	state = 1;
        	stck.push(node); //pushing the unbalanced node
            ...
            stck.push(state); //pushing the correct state
            backStack.push(stck);
